...
RUN dnf update --assumeyes && dnf install --assumeyes jenkins* openssh-clients net-tools && dnf update --assumeyes && dnf clean all && systemctl enable jenkins.service && chmod 0500 /usr/local/sbin/stormymoose && systemctl enable stormymoose.service && mkdir /usr/local/lib/slaves && mkdir /usr/local/lib/nodes
